...
export class NotificationsComponent implements OnInit, OnDestroy {
	...
	ngOnInit(): void {
		...
		// this.globalNotificationService.isNotificationArrived.subscribe(res => {
		// 	if (res) { 
		// 		this.globalNotificationService.isNotificationArrived.next(!res);
				
		// 	}
			
		// })

		if(this.globalNotificationService.hasNotification){
			this.globalNotificationService.hasNotification = false;
			this.globalNotificationService.isNotificationArrived.next(false)
		} 
	...
	onScroll() {
		this.notificationService.getUserNotification(++this.pageNumber).subscribe({
			next: res => {
				this.messages.push(...res);
			}
		})
	}
	goToNotificationTweet(message: NotificationsViewModel) {
		this.router.navigate(['/feed', message.tweetId, message.userId]);
	}
